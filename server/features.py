...
def process_categoricals(data):
    claim_reported_time = pd.to_datetime(
        data["claim_date_reported"], format=CLAIM_DATE_FORMAT
    )
    claim_occurred_time = pd.to_datetime(
        data["claim_date_occurred"], format=CLAIM_DATE_FORMAT
    )
    data["report_time_delta"] = (
        claim_reported_time - claim_occurred_time
    ).dt.total_seconds()
    data["occurred_date_season"] = pd.DatetimeIndex(claim_occurred_time).month // 4
    categoricals = CATEGORICALS
    for column in categoricals:
        data = data.join(
            pd.get_dummies(data[column].astype("category"), prefix=column), rsuffix="_"
        )
...
def make_vehicle_age_feature(data):
    ...
    data["vehicle_age_years"] = pd.DatetimeIndex(claim_occurred_time).year - data[
        "object_year_construction"
    ].astype(int)
    return data
...
def cleaned_features_from_string(input_string):
    df = pd.DataFrame(data=[input_string.split(",")], columns=COLUMNS)
    return process_features(df).astype('float32').to_numpy()
